...
class SensorPush : public PollingComponent {
 public:
  ...
  SensorPush(const std::string& mac, uint32_t interval) : PollingComponent(interval), mac_address(mac), polling_interval_ms(interval) {}
  ...
  void update() override {
    ...
    if (!pClient->connect(mac_address.c_str())) {
    ...
    BLERemoteCharacteristic* pHumharacteristic = pRemoteService->getCharacteristic(humUUID);
    ...
    bat_sensor->publish_state(convertTemperature(batValue));
